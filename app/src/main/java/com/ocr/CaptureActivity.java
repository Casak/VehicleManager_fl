...
import com.camera.ShutterButton;
import com.ocr.OcrResult;
import com.ocr.ActivityVehicleManager;
import com.language.LanguageCodeHelper;
...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback, 
  ...
  private static final int OPTIONS_SAVE_RECOGNIZED_TEXT_ID = Menu.FIRST + 2;
  ...
  private int _ocrEngineMode = TessBaseAPI.OEM_TESSERACT_ONLY;
...
//  private ToggleButton torchButton;
  ...
  private int langSelectIndex;

  private String _strFolderPathSave;
  private String _strFolderPathTessData;
  ...
  protected void onResume() {
    ...
    if (getIntent() != null && getIntent().getExtras() != null){
    	langSelectIndex = getIntent().getExtras().getInt("langIndex");
    }
    ...
    if (doNewInit) { 
      Log.d(TAG, "init new tesseract ocr");
    
      	// Initialize the OCR engine
      isEngineReady = initTessBaseAPI(sourceLanguageCodeOcr, _ocrEngineMode);

      //File storageDirectory = getStorageDirectory();
      //if (storageDirectory != null) {
		//strSaveFolderPath = storageDirectory.toString();
      ...
      resumeOCR();
      Log.d(TAG, "tesseract ocr is already exist.");
    }
  }
  ...
  void resumeOCR() {
    ...
    if (handler != null) {
    	Log.d(TAG, "handler.resetState()");
      handler.resetState();
    }
    if (baseApi != null) {
    	Log.d(TAG, "baseApi.setPageSegMode");
      baseApi.setPageSegMode(pageSegmentationMode);
      baseApi.setVariable(TessBaseAPI.VAR_CHAR_BLACKLIST, "0123456789");//characterBlacklist
      baseApi.setVariable(TessBaseAPI.VAR_CHAR_WHITELIST, "0123456789");//characterWhitelist
    }
  ...
  private boolean initTessBaseAPI(String languageCode, int ocrEngineMode ) {
	  //String strDir = getFilesDir().getPath() + "/" + WelcomeActivity.ENGINE_DIR + "te";
	  //File directory = new File(getFilesDir(), WelcomeActivity.ENGINE_DIR);
	  //String targetPath = directory.getAbsolutePath();// + WelcomeActivity.ENGINE_FILE + ".traindata";
		//String strPathStorage = getStorageDirectory().toString() + File.separator;//Environment.getExternalStorageDirectory().toString();
		//String targetPath = strPathStorage;// + "tessdata" + File.separator;
		
        String strDirTessData = getFilesDir().toString() + File.separator;// + WelcomeActivity.DIR_TESSDATA;
        File fileTessdataDir = new File(strDirTessData);// new File(targetPath)
		
		Log.e(TAG, "==========================");
		Log.e(TAG, "======" + strDirTessData + "=======");
		Log.e(TAG, "==========================");
		if ( fileTessdataDir.exists() == false ) {
			Log.e(TAG, "the tessdata directory : " + strDirTessData + " is not existed.");
	    	showErrorMessage("Error", "the tessdata directory : " + strDirTessData + " is not existed.");
			//tessdataDir.mkdirs();
			return false;
		}
    ...
    try {
	  ...
	  if( baseApi == null)
	  {
	    	Log.e(TAG, "Fail to create TessBaseAPI()");
	    	showErrorMessage("Error", "Fail to create TessBaseAPI().");
	    	return false;
	  }
	  else
	  {
		  boolean bInitTessBase = baseApi.init(strDirTessData, "eng", 0);//, ocrEngineMode);
		  //boolean bInitTessBase = baseApi.init(targetPath, languageCode, ocrEngineMode);
		  if( bInitTessBase == false)
		  {
		      showErrorMessage("Error", "Fail to initialize TessBaseAPI.");
		      return false;
		  }

		  Log.e(TAG, "Success TessBaseAPI initializaition.");
	  	  return true;
	  }
    } catch (RuntimeException e) {
	      // Barcode Scanner has seen crashes in the wild of this variety:
  ...
  private void initOcrEngine(File storageRoot, String languageCode, String languageName) {    
    ...
    if (_ocrEngineMode != TessBaseAPI.OEM_CUBE_ONLY) {
      for (String s : CUBE_REQUIRED_LANGUAGES) {
        if (s.equals(languageCode)) {
          _ocrEngineMode = TessBaseAPI.OEM_CUBE_ONLY;
    ...
    if (_ocrEngineMode != TessBaseAPI.OEM_TESSERACT_ONLY) {
      ...
      if (!cubeOk) {
        _ocrEngineMode = TessBaseAPI.OEM_TESSERACT_ONLY;
        //SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
        //prefs.edit().putString(PreferencesActivity.KEY_OCR_ENGINE_MODE, getOcrEngineModeName()).commit();
      }
    }
    ...
    if (_ocrEngineMode == TessBaseAPI.OEM_CUBE_ONLY || _ocrEngineMode == TessBaseAPI.OEM_TESSERACT_CUBE_COMBINED) {
    ...
    baseApi = new TessBaseAPI();
    new OcrInitAsyncTask(this, baseApi, dialog, indeterminateDialog, languageCode, languageName, _ocrEngineMode)
  ...
  boolean handleOcrDecode(OcrResult ocrResult) {
	...
	Intent intent = new Intent(CaptureActivity.this, ActivityVehicleManager.class);
	//Intent intent = new Intent(CaptureActivity.this, WelcomeActivity.class);
	intent.putExtra("recog_text", ocrResult.getText());
...
*/
  ...
  public void onCreateContextMenu(ContextMenu menu, View v,
    ...
    if (v.equals(ocrResultView)) {
      ...
      menu.add(Menu.NONE, OPTIONS_SAVE_RECOGNIZED_TEXT_ID, Menu.NONE, "Save recognized text");
  ...
  private void SaveRecognizedText()
  {
	  ...
	  String strSavePath = _strFolderPathSave + File.separator + strDate + ".txt";
	  
	  String strRecognizedText = ocrResultView.getText().toString();
...
//	  }
  ...
  String getOcrEngineModeName() {
    ...
    } else if (_ocrEngineMode == TessBaseAPI.OEM_CUBE_ONLY) {
      ocrEngineModeName = ocrEngineModes[1];
    } else if (_ocrEngineMode == TessBaseAPI.OEM_TESSERACT_CUBE_COMBINED) {
